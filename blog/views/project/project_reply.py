...
class ProjectReplyCreateJsonView(LoginRequiredMixin, View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if parent_pk :
            parent = ProjectPostReply.objects.get(pk=parent_pk)
        else :
            parent = None
        ...
        if parent :
            ...
            with transaction.atomic():
                ProjectPost.objects.select_for_update().filter(pk=kwargs.get("pk")).update(last_group_num=F('last_group_num') + 1)
                post = get_object_or_404(ProjectPost,pk=kwargs.get("pk"))
            group=post.last_group_num

        with transaction.atomic():
            ProjectPostReply.objects.create(author=author, comment=comment, depth=depth, group=group,  parent=parent, post=post)
            ProjectPost.objects.select_for_update().filter(pk=kwargs.get("pk")).update(reply_count=F('reply_count') + 1)
...
class ProjectReplyUpdateJsonView(LoginRequiredMixin,View):
    def post(self, request, pk, reply_pk):
        ...
        reply = get_object_or_404(ProjectPostReply,pk=reply_pk)
...
class ProjectReplyDeleteView(LoginRequiredMixin, DeleteView):
    ...
    def get_success_url(self):
        return reverse_lazy('blog:project_detail', kwargs={'pk': self.post_pk})