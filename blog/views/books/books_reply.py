    ...
    def post(self, request, *args, **kwargs):
        ...
        if parent :
            ...
            with transaction.atomic():
                BooksPost.objects.select_for_update().filter(pk=kwargs.get("pk")).update(last_group_num=F('last_group_num') + 1)
                post = get_object_or_404(BooksPost,pk=kwargs.get("pk"))
            group=post.last_group_num

        with transaction.atomic():
            BooksPostReply.objects.create(author=author, comment=comment, depth=depth, group=group,  parent=parent, post=post)
            BooksPost.objects.select_for_update().filter(pk=kwargs.get("pk")).update(reply_count=F('reply_count') + 1)
