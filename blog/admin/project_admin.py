...
class ProjectPostAdmin(AdminCommonAction, AdminCacheCleanPost):
    list_per_page = 20
    ...
    def get_actions(self, request):
        actions = super(ProjectPostAdmin, self).get_actions(request)
...
class ProjectPostReplyAdmin(AdminCommonAction, AdminCacheCleanReply):
    list_per_page = 20
    ...
    def get_actions(self, request):
        actions = super(ProjectPostReplyAdmin, self).get_actions(request)
        del actions["delete_selected"]
        return actions
