...
class Post(models.Model):
    ...
    file1 = models.FileField(
        upload_to=date_upload_to_for_file, blank=True, verbose_name=_("file1")
    )
    file2 = models.FileField(
        upload_to=date_upload_to_for_file, blank=True, verbose_name=_("file2")
    )
    table_name = models.CharField(
        max_length=30, blank=True, verbose_name=_("table_name")
    )
...
class ProjectPost(Post):
    ...
    like_user_set = models.ManyToManyField(
        ...
        through="blog.ProjectLike",
        verbose_name=_("Like Set"),
    )
    ...
    for field in instance._meta.fields:
        ...
        if (
            ...
            if origin_file != new_file and os.path.isfile(origin_file.path):
                ...
                logger.debug(
                    "updating {} field file are replacing  from = {}, to = {} at model of {}".format(
                        field_type, origin_file, new_file, sender.__name__
                    )
                )
        ...
        for field in instance._meta.fields:
            ...
            if (
                ...
                if origin_file and os.path.isfile(origin_file.path):
                    os.remove(origin_file.path)
                    logger.debug(
                        "{} field file is deleted name of {} at model of {}".format(
                            field_type, origin_file, sender.__name__
                        )
                    )
