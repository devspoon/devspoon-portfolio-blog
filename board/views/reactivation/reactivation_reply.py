...
class ReactivationReplyCreateJsonView(LoginRequiredMixin, View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if parent_pk :
            parent = ReactivationReply.objects.get(pk=parent_pk)
        else :
            parent = None

        if parent :
            group = parent.group
            post = get_object_or_404(Reactivation,pk=kwargs.get("pk"))
        else :
            with transaction.atomic():
                Reactivation.objects.select_for_update().filter(pk=kwargs.get("pk")).update(last_group_num=F('last_group_num') + 1)
                post = get_object_or_404(Reactivation,pk=kwargs.get("pk"))
            group=post.last_group_num

        with transaction.atomic():
            ReactivationReply.objects.create(author=author, comment=comment, depth=depth, group=group,  parent=parent, board=post)
            Reactivation.objects.select_for_update().filter(pk=kwargs.get("pk")).update(reply_count=F('reply_count') + 1)
...
class ReactivationReplyUpdateJsonView(LoginRequiredMixin,View):
    def post(self, request, pk, reply_pk):
        ...
        reply = get_object_or_404(ReactivationReply,pk=reply_pk)
...
class ReactivationReplyDeleteView(LoginRequiredMixin, DeleteView):
    ...
    def get_success_url(self):
        return reverse_lazy('board:reactivation_detail', kwargs={'pk': self.post_pk})