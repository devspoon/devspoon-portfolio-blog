    ...
    def __call__(self, email):
        try:
            ...
            raise ValidationError(
                "You must register first or try social login.", code=self.code
            )
        else:
            if user.verified is not True:
                raise ValidationError(
                    "You have to get email verification first", code=self.code
                )
...
class ResendVerificationEmailValidator:
    ...
    def __call__(self, email):
        try:
            ...
            raise ValidationError(
                "You must register first. You might be a social login user",
                code=self.code,
            )
        else:
            if user.verified is True:
                raise ValidationError(
                    "Your email verification is successfully finished", code=self.code
                )
...
class ResetPasswordEmailValidator:
    ...
    def __call__(self, email):
        try:
            ...
            raise ValidationError(
                "Your email information does not exist or try social login.",
                code=self.code,
            )
